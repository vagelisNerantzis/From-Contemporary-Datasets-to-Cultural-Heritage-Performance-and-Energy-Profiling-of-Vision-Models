...
def train_model(config):
    """
    Main function to train a MaxViT model using the timm library.
    ...
    Args:
        config (dict): Configuration dictionary loaded from a YAML file.
                       Contains parameters like model_name, data_dir, batch_size, etc.
    """
    ...
    # Create the model using the timm library.
    # config["model_name"] specifies the model architecture (e.g., 'maxvit_tiny_tf_512').
    # pretrained loads pre-trained weights if True.
    # num_classes sets the output layer size to match the number of classes in the dataset.
    ...
    for i in range(conf_matrix.shape[0]):
        for j in range(conf_matrix.shape[1]):
            ...
            # Add text with value and appropriate color, centering horizontally.
